    ...
    <div style={{ marginTop: "1rem" }}>
        ...
        {pasos.map((p, i) => (
          <button
            ...
            style={{
              ...
              border: `1px solid ${activo === i ? color : isLight ? "rgba(0,0,0,0.15)" : "rgba(255,255,255,0.15)"}`,
              background: activo === i ? `${color}22` : "transparent",
              color: activo === i ? color : isLight ? "rgba(0,0,0,0.45)" : "rgba(255,255,255,0.5)",
      ...
      <div
        style={{
          ...
          borderRadius: "6px", padding: "12px 16px", fontSize: "14px",
          color: isLight ? "rgba(0,0,0,0.75)" : "rgba(255,255,255,0.8)",
          lineHeight: "1.6", minHeight: "52px",
        }}
      >
        {pasos[activo].desc}
      </div>
...
function TipsAccordion({ tips, isLight }) {
...
function SeccionCard({ s, completada, onToggleCompleta, isLight }) {
  ...
  return (
    <div
      style={{
        ...
        border: `1px solid ${expandida ? s.color + "55" : isLight ? "rgba(0,0,0,0.1)" : "rgba(255,255,255,0.08)"}`,
    ...
    >
      ...
      >
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
            <span
              style={{
                fontFamily: "Bebas Neue, Orbitron, monospace", fontSize: "17px",
          ...
          <div
            style={{
              fontSize: "12px",
              color: isLight ? "rgba(0,0,0,0.4)" : "rgba(255,255,255,0.4)",
              fontFamily: "Rajdhani, sans-serif", marginTop: "2px",
            }}
          >
            {s.subtitulo}
          </div>
      ...
      {expandida && (
        <div style={{ padding: "0 20px 20px" }}>
          ...
          <TipsAccordion tips={s.tips} isLight={isLight} />
            ...
            <button
              ...
              style={{
                ...
                background: completada
                  ...
                  : `${s.color}22`,
                border: `1px solid ${completada ? isLight ? "rgba(0,0,0,0.15)" : "rgba(255,255,255,0.15)" : s.color + "66"}`,
                color: completada ? isLight ? "rgba(0,0,0,0.4)" : "rgba(255,255,255,0.4)" : s.color,
                padding: "6px 14px", borderRadius: "6px", cursor: "pointer", transition: "all 0.2s",
              }}
            >
...
export default function Guia() {
  ...
  const pageBg = isLight ? "#f9f9f7" : "#0f0f0f"
  const titleColor = isLight ? "#1a1a1a" : "#fff"
  const subtitleColor = isLight ? "rgba(0,0,0,0.45)" : "rgba(255,255,255,0.4)"
  const progressBg = isLight ? "rgba(0,0,0,0.05)" : "rgba(255,255,255,0.06)"
  const progressBorder = isLight ? "rgba(0,0,0,0.1)" : "rgba(255,255,255,0.1)"
  const searchBg = isLight ? "rgba(0,0,0,0.04)" : "rgba(255,255,255,0.05)"
  const searchBorder = isLight ? "rgba(0,0,0,0.12)" : "rgba(255,255,255,0.12)"
  const searchColor = isLight ? "#1a1a1a" : "#fff"
  const footerColor = isLight ? "rgba(0,0,0,0.2)" : "rgba(255,255,255,0.2)"
  ...
  return (
    <div style={{ minHeight: "100vh", background: pageBg, padding: "32px 24px", fontFamily: "Rajdhani, sans-serif" }}>
      <div style={{ maxWidth: "760px", margin: "0 auto" }}>
        ...
        <div style={{ marginBottom: "32px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "14px", marginBottom: "6px" }}>
            <img
              src="/logo.png" alt="LoreWeaver"
              style={{ width: "70px", height: "70px", objectFit: "contain", borderRadius: "10px", filter: logoFilter }}
            />
            <h1 style={{ fontFamily: "Bebas Neue, Orbitron, monospace", fontSize: "clamp(28px, 5vw, 42px)", letterSpacing: "0.12em", color: titleColor, margin: 0 }}>
              {t.guiaTitulo}
            </h1>
          </div>
          <p style={{ color: subtitleColor, fontSize: "14px", margin: 0 }}>{t.guiaSubtitulo}</p>
        </div>

        {/* Barra de progreso */}
        <div style={{ background: progressBg, border: `1px solid ${progressBorder}`, borderRadius: "10px", padding: "14px 18px", marginBottom: "24px", display: "flex", alignItems: "center", gap: "16px" }}>
          <div style={{ flex: 1 }}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "6px" }}>
              <span style={{ fontSize: "12px", color: subtitleColor, letterSpacing: "0.06em", textTransform: "uppercase" }}>
                {t.progresoLabel}
              </span>
              <span style={{ fontSize: "12px", color: isLight ? "rgba(0,0,0,0.65)" : "rgba(255,255,255,0.7)", fontWeight: 600 }}>
                {completadas.size}/{secciones.length} {t.seccionesLabel}
              </span>
            </div>
            <div style={{ height: "6px", background: isLight ? "rgba(0,0,0,0.08)" : "rgba(255,255,255,0.1)", borderRadius: "3px", overflow: "hidden" }}>
              <div style={{ height: "100%", width: `${progreso}%`, background: progreso === 100 ? "#34d399" : "#a78bfa", borderRadius: "3px", transition: "width 0.4s ease" }} />
            </div>
          </div>
          <span style={{ fontFamily: "Bebas Neue, monospace", fontSize: "22px", color: progreso === 100 ? "#34d399" : "#a78bfa", minWidth: "44px", textAlign: "right" }}>
            {progreso}%
          </span>
        </div>
        ...
        <div style={{ position: "relative", marginBottom: "20px" }}>
          <span style={{ position: "absolute", left: "14px", top: "50%", transform: "translateY(-50%)", color: subtitleColor, fontSize: "14px", pointerEvents: "none" }}>
            🔍
          </span>
          <input
            type="text"
            placeholder={t.buscarGuia}
            value={busqueda}
            ...
            style={{
              ...
              color: searchColor, fontSize: "14px",
        ...
        {/* Secciones */}
        {filtradas.length === 0 ? (
          <div style={{ textAlign: "center", padding: "48px 0", color: subtitleColor, fontSize: "14px" }}>
        ...
        {/* Footer */}
        <div style={{ marginTop: "40px", textAlign: "center", color: footerColor, fontSize: "12px", letterSpacing: "0.06em" }}>
          {t.footerGuia}
        </div>
      </div>
    </div>
