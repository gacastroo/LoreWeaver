...
export default function EditorUniverso({ onUpdate }) {
  ...
  if (loading) return <p className={`p-6 ${isLight ? "text-neutral-700" : "text-zinc-400"}`}>{t.cargandoUniverso}</p>;
  ...
  return (
    <div className={`p-8 max-w-4xl mx-auto min-h-screen ${isLight ? "bg-white" : "bg-zinc-950"}`}>
      {universo && (
        <div className={`flex flex-wrap gap-4 mb-6 border-b pb-4 ${isLight ? "border-neutral-200" : "border-zinc-700"}`}>
          {universo.historia && (
            ...
            >
              {t.irAHistoria} {universo.historia.titulo}
            </button>
          )}
        </div>
      )}
      ...
      <textarea
        ...
        placeholder={t.descripcionUniversoPlaceholder || "Escribe la descripción del universo..."}
