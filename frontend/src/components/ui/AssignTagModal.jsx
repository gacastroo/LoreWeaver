...
export default function AssignTagModal({ personajeId, onClose, onSuccess, tagsAsignados = [] }) {
  ...
  return (
    <AccessibleModal title="Asignar etiqueta" onClose={onClose} className="bg-white p-6 rounded-2xl shadow-lg w-full max-w-md border border-neutral-200">
        <label htmlFor="tag-asignado" className="sr-only">Etiqueta a asignar</label>
        <select
          id="tag-asignado"
          value={selectedTag}
          onChange={(e) => setSelectedTag(e.target.value)}
          className="w-full p-2 text-sm border border-neutral-300 rounded-md focus:outline-none focus:ring-2 focus:ring-neutral-500 focus:ring-offset-2 mb-6"
        >
